...
def main(file_path1: str, file_path2: str):
    ...
    only_1_passed = [t for t in only_1 if report1[t] >= 0]
    print()
    print(f"@@ NEW ({len(only_1_passed)} PASSED) @@")
    for test in only_1_passed:
        failed_attempts = report1[test]
        print(f"<{'-' if failed_attempts < 0 else '+'}{test}: {failed_attempts}")

    only_1_failed = [t for t in only_1 if report1[t] < 0]
    print()
    print(f"@@ NEW ({len(only_1_failed)} FAILED) @@")
    for test in only_1_failed:
        failed_attempts = report1[test]
        print(f"<{'-' if failed_attempts < 0 else '+'}{test}: {failed_attempts}")

    only_2_passed = [t for t in only_2 if report2[t] >= 0]
    print()
    print(f"@@ REMOVED ({len(only_2_passed)} PASSED) @@")
    for test in only_2_passed:
        failed_attempts = report2[test]
        print(f">{'-' if failed_attempts < 0 else '+'}{test}: {failed_attempts}")

    only_2_failed = [t for t in only_2 if report2[t] < 0]
    print()
    print(f"@@ REMOVED ({len(only_2_failed)} FAILED) @@")
    for test in only_2_failed:
        failed_attempts = report2[test]
        print(f">{'-' if failed_attempts < 0 else '+'}{test}: {failed_attempts}")

    improved_now_passes = [t for t in improved if report1[t] < 0]
    print()
    print(f"@@ Improved, now PASSED ({len(improved_now_passes)}) @@")
    for test in improved_now_passes:
        print(f"++{test}: {report1.get(test)} -> {report2.get(test)}")

    improved_minor = [t for t in improved if report1[t] >= 0]
    print()
    print(f"@@ Improved, minor ({len(improved_minor)}) @@")
    for test in improved_minor:
        print(f"+ {test}: {report1.get(test)} -> {report2.get(test)}")

    worsened_now_fails = [t for t in worsened if report2[t] < 0]
    print()
    print(f"@@ Worsened, now FAILED ({len(worsened_now_fails)}) @@")
    for test in worsened_now_fails:
        print(f"--{test}: {report1.get(test)} -> {report2.get(test)}")

    worsened_minor = [t for t in worsened if report2[t] >= 0]
    print()
    print(f"@@ Worsened, still PASSED ({len(worsened_minor)}) @@")
    for test in worsened_minor:
        print(f"- {test}: {report1.get(test)} -> {report2.get(test)}")

    stable_passed = [t for t in stable if report1[t] >= 0]
    print()
    ...
    stable_failed = [t for t in stable if report1[t] < 0]
    print()
    print(f"@@ Stable: FAILED ({len(stable_failed)}) @@")
    for test in stable_failed:
        failed_attempts_2 = report2.get(test)
        print(f"=-{test}: {report1.get(test)}{f" -> {failed_attempts_2}" if failed_attempts_2 is None or failed_attempts_2 < 0 else ''}")

    print()
    print("# =============          TOTALS          =============")
    print(f"# REMOVED : {len(only_1)}")
    print(f"#    PASSED: {len(only_1_passed)}")
    print(f"#    FAILED: {len(only_1_failed)}")
    print(f"# NEW     : {len(only_2)}")
    print(f"#    PASSED: {len(only_2_passed)}")
    print(f"#    FAILED: {len(only_2_failed)}")
    print(f"# IMPROVED: {len(improved)}")
    print(f"#    Now PASSES: {len(improved_now_passes)}")
    print(f"#    Minor     : {len(improved_minor)}")
    print(f"# WORSENED: {len(worsened)}")
    print(f"#    Now FAILED: {len(worsened_now_fails)}")
    print(f"#    Minor     : {len(worsened_minor)}")
    print(f"# STABLE  : {len(stable)}")
    print(f"#    PASSED: {len(stable_passed)}")
    print(f"#    FAILED: {len(stable_failed)}")
